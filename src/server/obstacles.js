...
class Obstacle {
    constructor(params){
        ...
        /*if(this.type.includes('turret')){
            this.projectiles = [];
            this.timer = params.shootTimer;
            this.shootInd = params.startIndex;
        }*/
    ...
    update(dt){
        ...
        /*if(this.type.includes('turret')){
            this.timer -= dt;
            if(this.timer < 0){
                this.timer += this.shootTimer;
                // shoot bullet
                this.projectiles.push({x: this.y, lifespan: this.lifespan, y: this.y, speed: this.projectileSpeed, radius: this.projectileRadius, angle: this.shootAngles[this.shootInd]});
                this.shootInd++;
                if(this.shootInd+1 >= this.shootAngles.length){
                    this.shootInd = 0;
                }
            }
        }
        if(this.type.includes('projectile')){
            this.lifespan -= dt;
            if(this.lifespan <= 0){
                delete this;
            }
            let xv = Math.cos(this.angle) * this.speed;
            let yv = Math.sin(this.angle) * this.speed;
            this.x += xv * dt;
            this.y += yv * dt;
        }*/
    }
